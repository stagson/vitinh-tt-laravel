...
class PhieuXuat extends Model
{
    ...
    public function doanhThuTuanNay(){
        ...
        $doanhThu=DB::select('SELECT DAY(`ngaytao`) AS ngay, SUM(`tongtien`) AS doanhthu FROM phieuxuat WHERE tinhtranggiaohang = 4 AND DAY(`ngaytao`) BETWEEN ? AND ? GROUP BY ngay',$data);
        if(!empty($doanhThu)){
            $data=[];
